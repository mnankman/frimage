...
class PersistentObject:
    ...
    def setDataAttribute(self, attr, attrValue, typeName=None):
        className = self.getType() if typeName==None else typeName
        names = ["_"+className+"__"+attr]
        # construct a list of possible qualified names (class.attribute)
        for base in self.__class__.__bases__:
            names.append("_"+base.__name__+"__"+attr)
        # try each of these names
        for nm in names:
            # see if the class has a setter method for this attribute
            setterName = "set"+util.upperFirst(attr)
